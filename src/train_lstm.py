...
class LSTM:
	def __init__(self, args, data, tuning):
		...
		# self.total_train_loss = tf.Variable(0)
		# self.total_train_acc = tf.Variable(0)
		...
		pred = self.model.inference(foward_only=True)
		self.test_loss = self.model.loss(pred, foward_only=True)
